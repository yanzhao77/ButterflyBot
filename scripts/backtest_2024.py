"""
ButterflyBot 2024年专项回测脚本
使用新模型（v20260427_145956，5年训练，Platt校准）
数据范围：2024-01-01 ~ 2024-12-31（约 35,040 根 K 线）
...
print("=" * 70)
print(f"🚀 ButterflyBot {TARGET_YEAR}年专项回测")
print("=" * 70)
...
print(f"   总时间范围: {df_all.index[0]} ~ {df_all.index[-1]}")
...
# ===================== 4. 截取目标年份数据 =====================
mask_year = df_all.index.year == TARGET_YEAR
df_year   = df_all[mask_year].copy()
pema_year = pema_all[mask_year]

print(f"\n📅 {TARGET_YEAR}年数据: {len(df_year)} 根K线")
print(f"   时间范围: {df_year.index[0]} ~ {df_year.index[-1]}")
print(f"   DOGE价格范围: min={df_year['close'].min():.4f}, max={df_year['close'].max():.4f}")
print(f"   p_ema 统计: min={pema_year.min():.4f}, max={pema_year.max():.4f}, mean={pema_year.mean():.4f}")
print(f"   超阈值0.35次数: {(pema_year >= 0.35).sum()}")
print(f"   超阈值0.40次数: {(pema_year >= 0.40).sum()}")
print(f"   超阈值0.45次数: {(pema_year >= 0.45).sum()}")

# ===================== 5. 回测引擎 =====================
def run_backtest(df, pema, label, conf_th, sl_pct, tp_pct, pos_pct, use_hard_stop=True):
...
# ===================== 6. 运行3组回测 =====================
print("\n" + "=" * 70)
print(f"📊 运行{TARGET_YEAR}年回测（3组参数对比）...")
print("=" * 70)

configs = [
    ("A.当前配置(0.40,-2%,+4%)",   0.40, -0.02, 0.04),
    ("B.宽松阈值(0.35,-2%,+4%)",   0.35, -0.02, 0.04),
    ("C.收紧止损(0.40,-1.5%,+3%)", 0.40, -0.015, 0.03),
]

results = []
for label, th, sl, tp in configs:
    print(f"\n[{label[0]}] {label}")
    r = run_backtest(df_year, pema_year, label, th, sl, tp, POSITION_PCT)
    results.append(r)
...
ax1_r = ax1.twinx()
ax1_r.fill_between(df_year.index, df_year["close"].values, alpha=0.07, color="#e3b341")
ax1_r.plot(df_year.index, df_year["close"].values, color="#e3b341", linewidth=0.8, alpha=0.5)
...
for t in rA["trades"]:
    ei = t["entry_idx"]
    if ei < len(df_year):
        ax1.axvline(df_year.index[ei], color="#58a6ff", linewidth=0.4, alpha=0.25)

ax1.set_title(f"{TARGET_YEAR}年回测权益曲线（3组参数对比）", fontsize=12)
...
if rA["reason_counts"]:
    ...
    for at in autotexts:
        at.set_color(C_TEXT); at.set_fontsize(8)
...
summary_text = (
    f"{TARGET_YEAR}年回测汇总\n"
    f"{'─'*28}\n"
    f"数据: {TARGET_YEAR}-01-01 ~ {TARGET_YEAR}-12-31\n"
    f"K线: {len(df_year):,} 根 (15m)\n"
    f"{'─'*28}\n"
...
fig.suptitle(
    f"ButterflyBot {TARGET_YEAR}年专项回测\n"
    f"DOGE/USDT 15m | 新模型 v20260427 | 初始资金: 1,000 USDT",
    fontsize=13, color=C_TEXT, y=0.99
)

chart_path = f"reports/backtest/backtest_{TARGET_YEAR}_chart.png"
...
report_data = {
    ...
    "symbol":    "DOGE/USDT",
    "timeframe": "15m",
    "period":    f"{TARGET_YEAR}-01-01 ~ {TARGET_YEAR}-12-31",
    "total_bars": len(df_year),
...
json_path = f"reports/backtest/backtest_{TARGET_YEAR}_{timestamp}.json"
with open(json_path, "w", encoding="utf-8") as f:
    json.dump(report_data, f, ensure_ascii=False, indent=2)
print(f"✅ JSON报告已保存: {json_path}")

# ===================== 11. p_ema 分布分析 =====================
print(f"\n📊 {TARGET_YEAR}年 p_ema 分布分析:")
for q, label in [(0.75, "75%分位"), (0.85, "85%分位"), (0.90, "90%分位"), (0.95, "95%分位")]:
    print(f"   {label}:    {np.quantile(pema_year, q):.4f}")
print(f"   最大值:     {pema_year.max():.4f}")

print("\n" + "=" * 70)
print(f"✅ {TARGET_YEAR}年回测完成！")
best = max(results, key=lambda r: r["net_return_pct"])
print(f"   最优配置: {best['label']}")
print(f"   净收益={best['net_return_pct']:+.2f}%, 最大回撤={best['max_drawdown_pct']:.2f}%, 交易{best['total_trades']}笔, 胜率{best['win_rate']:.2f}%")
print("=" * 70)
